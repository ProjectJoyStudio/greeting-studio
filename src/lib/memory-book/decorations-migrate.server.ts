// Server-only: moves the Project Joy decorations library into the working
// storage area.
//
...
export function decorationKey(id: string, path: string, fileType: string): string {
 ...
 */
export async function migrateDecorationsToPrimary(): Promise<{
  ok: boolean;
  items: DecorationMigrationItem[];
  error?: string;
}> {
  ...
  for (const raw of ((data ?? []) as unknown as Row[])) {
    ...
    // Already on the working area: only confirm it is really there and that
